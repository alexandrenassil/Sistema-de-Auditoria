<?php
  ...
  /* Parametros do farol
    ...
    */
   $script = "SELECT CASE WHEN CAST(GETDATE()-INICIO_FILA AS FLOAT) <= '0.00277777777777778' THEN 'Ate 4min' WHEN CAST(GETDATE()-INICIO_FILA AS FLOAT) <= '0.00347222222222222' THEN 'Entre 4:01min e 5min' ELSE 'Acima de 5:01min' END Farol,COUNT(1) QTD FROM AUDITORIA_VENDAS.DBO.CONTROLE_AUDITORIA_EM_EXECUCAO WITH (NOLOCK) WHERE DATEDIFF(D, INICIO_FILA, GETDATE()) = 0 AND FLAG_CONTROLE = 0 ".$filtroCampanha."".$filtroSupervisor." GROUP BY CASE WHEN CAST(GETDATE()-INICIO_FILA AS FLOAT) <= '0.00277777777777778' THEN 'Ate 4min' WHEN CAST(GETDATE()-INICIO_FILA AS FLOAT) <= '0.00347222222222222' THEN 'Entre 4:01min e 5min' ELSE 'Acima de 5:01min' END ";
